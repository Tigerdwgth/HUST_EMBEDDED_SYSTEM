...
uint8_t * pdm_to_pcm_s16_mono(uint8_t *pdm_buf, int pdm_bype_len, int pdm_sample_rate, int decimation, pcm_info_st *pcm_info)
{
	...
	if((decimation != 64)&&(decimation != 128)) {
		printf("decimation %d error\n", decimation);
		return NULL;
	}

	//PCM采样固定是s16/mono, 2byte
	pcm_info->numChannels = 1;
	pcm_info->bitsPerSample = 16;
    pcm_info->sampleRate = pdm_sample_rate / decimation;
    int pcm_sample_len = (pdm_bype_len * 8) / decimation;
    pcm_info->numBytes = pcm_sample_len * 2;

	uint8_t *pcm_buf = malloc(pcm_info->numBytes);
	if(pcm_buf == NULL) {
    ...
    filter.Fs = pcm_info->sampleRate;
    filter.nSamples = pcm_sample_len;
    filter.LP_HZ = pcm_info->sampleRate / 2;
    filter.HP_HZ = 10;
	...
	if(decimation == 64)
		Open_PDM_Filter_64(pdm_buf, (int16_t *)pcm_buf, 1, &filter);
	else
		Open_PDM_Filter_128(pdm_buf, (int16_t *)pcm_buf, 1, &filter);
	return pcm_buf;
}
...
uint8_t * pcm_s16_mono_resample(uint8_t *src_buf, pcm_info_st *src_info, int dst_sample_rate, pcm_info_st *pcm_info)
{
	...
	uint32_t src_sample_n = pcm_get_sample_num(src_info);
	uint32_t dst_sample_n =  src_sample_n * dst_sample_rate / src_sample_rate;

	pcm_info->numChannels = 1;
	pcm_info->bitsPerSample = 16;
    pcm_info->sampleRate = dst_sample_rate;
    pcm_info->numBytes = dst_sample_n * 2;
